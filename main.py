...
async def add_user(ism: str, familiya: str, user_id: str, file: UploadFile = File(...)):
    ...
    if file_extension not in ['jpg', 'jpeg', 'png']:
        raise HTTPException(status_code=400, detail="Faqat JPG, JPEG, yoki PNG formatidagi rasm fayllari qo'llab-quvvatlanadi.")
...
async def get_user(user_id: str):
    ...
    rasm_yoli = f"http://localhost:8084/download/rasm/{user_id}"
    return {
...
async def download_rasm(user_id: str):
    ...
    # Rasm formatiga qarab media_type ni aniqlash
    file_extension = rasm_path.split('.')[-1].lower()
    media_type = 'image/jpeg' if file_extension in ['jpg', 'jpeg'] else 'image/png'

    return FileResponse(rasm_path, media_type=media_type, filename=f"{user_id}.{file_extension}")
